...
function kebabToPascalWithoutExtension(filename)
  -- Удаляем расширение
...
local function call_python(python_function_name, opts)
  ...
  local result = vim.fn.py3eval(
    "luasnip_snippets_python_helper."
      .. python_function_name
      .. '(**(__import__("vim").vars.get("snip_utils_kwargs", {})))'
  )
...
local function code_python(id, node_code, global_code, args, snip, indent, tabstops_idx)
  ...
  return call_python("execute_code", {
    node_id = id,
    node_code = node_code,
